...
run "unit_validate_resource_group_configuration" {
  ...
  # Expected format: rg-<env>-<region>-<instance_number>
  # Example: rg-app-test-eu-001
  assert {
    condition     = can(regex("^rg-[a-z0-9]+-(dev|test|prod)-[a-z0-9]+-[0-9]{3}$", var.resource_group_name))
    error_message = "Resource Group name does not follow enterprise naming convention: rg-<application>-<environment>-<region>-<instance_number>."
  }
  ...
  assert {
    condition     = contains(["dev", "test", "prod"], lower(var.resource_group_tags["Environment"]))
