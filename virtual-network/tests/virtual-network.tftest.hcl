...
run "unit_validate_virtual_network_configuration" {
  ...
  assert {
    condition     = azurerm_virtual_network.virtual_network.name == var.name
    error_message = "Virtual Network name did not match as expected."
  }

  assert {
    condition     = azurerm_virtual_network.virtual_network.resource_group_name == var.resource_group_name
  ...
  # location comparison
  assert {
    condition     = lower(azurerm_virtual_network.virtual_network.location) == lower(replace(var.location, " ", ""))
  ...
  assert {
    condition     = can(regex("^vnet-[a-z0-9]+-(dev|test|prod)-[a-z0-9]+-[0-9]{3}$", var.name))
    error_message = "Virtual network name does not follow enterprise naming convention: vnet-<application>-<environment>-<region>-<instance_number>."
  }
