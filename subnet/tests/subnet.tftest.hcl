...
run "unit_validate_subnet_configuration" {
  ...
  assert {
    condition     = can(regex("^snet-[a-z0-9]+-(dev|test|prod)-[a-z0-9]+-[0-9]{3}$", var.name))
    error_message = "Subnet name does not follow enterprise naming convention: vnet-<application>-<environment>-<region>-<instance_number>."
  }
